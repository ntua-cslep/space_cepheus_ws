...
* **MT Software Suite**:
  ...
  * MT Manager (device verification, configuration, firmware)
  * Firmware Updater
...
## 10. MT Manager Verification Workflow (Phase 2)

### Purpose

Vendor-provided tooling was used to verify correct operation, firmware status, and baseline configuration of the Xsens Sirius IMU prior to any experimental configuration changes. This establishes a known-good reference state independent of the ROS software stack.

### Software and Connection

* **Tool:** Xsens MT Manager (MT Software Suite)
* **Operating system:** Windows
* **Connection:** RS-232 via USB (FTDI converter)

### Device Detection

The device was automatically detected by MT Manager with the following parameters:

* **Product code:** S1A43A
* **Hardware revision:** 1.2
* **Firmware revision:** 1.4.1 (build 71175, revision 162289791)
* **Communication interface:** RS-232
* **Baud rate:** 921600
* **Location ID:** 0
* **Filter profile:** Responsive (North-referenced)

### Live Data Verification

Live inertial data streaming was enabled using MT Manager visualization tools. Angular velocity, linear acceleration, and orientation outputs were monitored while manually exciting the robot base. All signals responded immediately and consistently to applied motion. When stationary, acceleration magnitude remained close to 1 g and angular velocity signals exhibited low noise and zero-mean behavior, confirming correct sensor operation and internal calibration.

### Baseline Configuration State

At the time of verification, the device was operating with magnetically referenced AHRS functionality enabled. This configuration was retained unchanged during Phase 2 and serves only as a factory baseline reference.

### Configuration Export

The complete device configuration was exported using MT Manager and archived as an `.xsa` file. This file represents the factory baseline configuration and is retained as a rollback reference prior to any reconfiguration performed in later phases.

### Scope and Limitations

During Phase 2, MT Manager was used exclusively for device detection, firmware verification, live data inspection, and configuration export. No changes were made to operating mode, filter behavior, output configuration, synchronization options, or modeling parameters. Configuration decisions are deferred to Phase 3.
