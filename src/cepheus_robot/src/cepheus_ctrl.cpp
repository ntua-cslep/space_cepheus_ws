...
void startCtrl(const std_msgs::StringConstPtr &msg, ros::NodeHandle &n, ros::Publisher &ctl_pub)
{
	...
	if(!standard_ctrls_started && ((msg->data).compare("START_STANDARD_CTRLS")== 0)){
		...
		if(rv)
			ROS_WARN("RIGHT ELBOW LOADED");
		else
			ROS_WARN("could not load right elbow");
		...
		rv = startControllers(n, v);
		if(rv)
			ROS_WARN("RIGHT ELBOW STARTED");
		else
			ROS_WARN("could not start right elbow");
		...
		// to_stop.push_back(std::string(LEFT_ELBOW_CONTROLLER));// tha to balo
		...
		if(rv)
			ROS_WARN("left shoulder unLOADED");
		else
			ROS_WARN("could not load");
		...
		to_start.push_back(std::string(LEFT_ELBOW_EFFORT_CONTROLLER)); //tha to balo (nikiforos)
