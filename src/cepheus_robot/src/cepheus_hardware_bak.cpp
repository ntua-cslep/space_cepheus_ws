...
void CepheusHW::init_right_elbow() {
	...
	if (home_pos[RIGHT_ELBOW]) {
		...
		ROS_WARN("homing RIGHT ELBOW.............."); //ROS_INFO_STREAM
		...
		do{
			...
			cmd[RIGHT_ELBOW] = 0.00001;// right_elbow_out; //nikiforos2025
			// 2020
			update_left_elbow(vel[LEFT_ELBOW], des_left_elbow, left_elbow_out, 0.5);
			// cmd[LEFT_ELBOW] = left_elbow_out; //nikiforos 2025
...
const int CepheusHW::logic[8] = {1, 0, 0, 0, -1, 1, 0, -1};

void CepheusHW::writeMotors()
{
	//ROS_INFO(" PWM_M_R  %d PWM_M_MIN %d\n",PWM_MOTOR_RANGE,PWM_MOTOR_MIN_DT);
	// cmd[LEFT_SHOULDER]=0.0000001; //NIKIFOROS APLA BLEPO KATI
	// cmd[RIGHT_ELBOW] = -0.0000000001;
	// ROS_INFO("[In cepheusHW::writeMotors(): cmd of Left shoulder is %f:]", cmd[LEFT_SHOULDER]);
	// ROS_INFO("[In cepheusHW::writeMotors(): cmd of Right elbow is %f:]", cmd[RIGHT_ELBOW]);
	//REACTION WHEEL
	...
	for (int i=4; i<8; i++)
	{
		...
		// Current w\ No load current, gear ratio, gearhead efficiency
		current[i] = Io + cmd[i] / Keff;

		// Clamp to max_current
		eff[i] = cmd[i];
		if (current[i] >= max_current[i]) {
			current[i] = max_current[i];
			eff[i] = (current[i] - Io) * Keff;
		}
	...
	// width[RIGHT_ELBOW] = 493;  //NIKIFOROS OVERRIDE sto 493 isa isa pou kineitai, kalo to 490
	// width[LEFT_SHOULDER] = 492;
	//width[RIGHT_ELBOW] -=10;
	//width[LEFT_SHOULDER] -=10;
	//width[LEFT_ELBOW] -=10;

	// ROS_INFO("[In cepheusHW::writeMotors(): PWM width of Left shoulder is %d:]", width[LEFT_SHOULDER]);
	// ROS_INFO("[In cepheusHW::writeMotors(): PWM width of Right elbow is %d:]", width[RIGHT_ELBOW]);
...
{
	//return 1; //bypass, nikiforos 2025
	return limit[i];
}
...
void CepheusHW::readEncoders(ros::Duration dt)
{
	...
	for(int i=0; i<8; i++)
	{
		double bilinear_fltr = 0.95;	// range from 0 to 1, it smoothens noise from numerical differentiation using previous vel


		//vel_new[i] = bilinear_fltr * vel[i] + (1 - bilinear_fltr) * ((pos[i] - prev_pos[i])) / dt.toSec();
		// 2025_12_03_bilinear filtr

		// before filtering, simple numerical differentiation
		vel_new[i] = ((pos[i] - prev_pos[i])) / dt.toSec();
		vel_new[i] = bilinear_fltr * vel[i] + (1-bilinear_fltr) * vel_new[i];
