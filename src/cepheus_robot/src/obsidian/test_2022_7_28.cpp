...
double acc3_x = 0.0, acc3_z = 0.0;
double acc3_y = 0.0, acc3_y_offset = 0.0, acc3_y_sum = 0.0;
double acc3_x_sum = 0.0, acc3_z_sum = 0.0;
double acc3_x_offset = 0.0, acc3_z_offset = 0.0;

double acc1_x = 0.0, acc1_z = 0.0;
double acc1_y = 0.0, acc1_y_offset = 0.0, acc1_y_sum = 0.0;
double acc1_x_sum = 0.0, acc1_z_sum = 0.0;
double acc1_x_offset = 0.0, acc1_z_offset = 0.0;

double acc2_x = 0.0, acc2_z = 0.0;
double acc2_y = 0.0, acc2_y_offset = 0.0, acc2_y_sum = 0.0;
double acc2_x_sum = 0.0, acc2_z_sum = 0.0;
double acc2_x_offset = 0.0, acc2_z_offset = 0.0;
...
void acc1YCallback(const std_msgs::Float32::ConstPtr& cmd) {
    acc1_y = cmd->data - acc1_y_offset;
}
...
void acc2YCallback(const std_msgs::Float32::ConstPtr& cmd) {
    acc2_y = cmd->data - acc2_y_offset;
}
...
void acc3YCallback(const std_msgs::Float32::ConstPtr& cmd) {
    acc3_y = cmd->data - acc3_y_offset;
}
...
int main(int argc, char** argv) {
	...
	ros::Subscriber acc_3_y_sub = nh.subscribe("/arduino_3/acc3_y", 1, acc3YCallback);
	ros::Subscriber acc_2_y_sub = nh.subscribe("/arduino_1_2/acc2_y", 1, acc2YCallback);
    ...
    ros::Publisher acc2_y_pub = nh.advertise<std_msgs::Float64>("acc2_y", 1);
    ...
    ros::Publisher acc3_y_pub = nh.advertise<std_msgs::Float64>("acc3_y", 1);
	...
	double theta3init = 0.0;
	double theta1fin = 0.0;
	double theta2fin = 0.0;
	double theta3fin = 0.0;
    ...
    double theta3_des = 0.0;
	double theta3Dot_des = 0.0;
	double theta3DotDot_des = 0.0;
	double theta0_des, theta0Dot_des, theta0DotDot_des;
	double ps_xdot, ps_ydot;
	double yamp = 0.3;
	double vt1, xt1, xt2, vt2;//ASTRA 2022

	double m0 = 12.0, m1 = 0.4409, m2 = 0.3874, m3 = 0.09;
	...
	double I1z = 0.0068;
	double I2z = 0.0010;
	double I3z = 0.0787;//********************these values are from EROSS need to be changed!!!!!*************
	...
	while 
	// (ls_position == 0.0 || le_position == 0.0 || 
	(ps_theta == 0.0 || angular_vel_z == 0.0 || acc_x == 0 || acc_y == 0) {
...
/////////////////////////////////////////////////// INITIAL CONDITIONS //////////////////////**********************************///////////////////////////////////////////////
		...
		if (first_time_movement) {
			...
			vt1 = 0.05;
			t11 = (vt1 - vpx_in)/a;
			xt1 = rpx_in + 0.5*a*pow(t11,2);
			xt2 = 1.6;
			t12 = (xt2 - xt1 + vt1*t11)/vt1;
			vt2 = vt1;
			tf = (vt2 - vpx_tar + a*t12)/a;
		...
		if (t <= tf ) {
			...
			theta1_des = 0.0;
			theta1Dot_des = 0.0;
			theta1DotDot_des = 0.0;
			theta2_des = 0.0;
			theta2Dot_des = 0.0;
			theta2DotDot_des = 0.0;
			...
			// ASTRA 2022
			theta0_des = theta0_in;
			theta0Dot_des = 0.0;
			theta0DotDot_des = 0.0;
			...
			if (t<=t11){
				...
				baseposx_des = xt1 + vt1*(t-t11);
				baseposxDot_des = vt1;
				baseposxDotDot_des = 0.0;
			}
			else {
				baseposx_des = xt2 + vt2*(t-t12) - 0.5*a*pow((t-t12),2);
				baseposxDot_des = vt2 - a*(t-t12) ;
				baseposxDotDot_des = -amax;
			}
