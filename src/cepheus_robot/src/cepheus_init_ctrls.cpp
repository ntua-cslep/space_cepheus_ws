...
void ctlNodeReport(const std_msgs::StringConstPtr &msg){
	...
	if(!standard_controllers_started && (msg->data).compare("STANDARD_CTRLS_OK") == 0){
		...
		ROS_WARN("STARTEEEEDD"); //TO anoiksa, nikiforos 2025
	}
										...
										ros::Rate& loop_rate)
{
	...
	robot.init_left_elbow();  //nikiforos 2025 to eixa balei se sxcolio gia ligo
										...
										ros::Rate& loop_rate)
{
	...
	robot.init_right_elbow();  //nikiforos 2025
...
//	ros::Time prev_time = update_time;
	...
	robot.init_left_elbow(); //nikiforos 2025 se sxolio 
										...
										ros::Rate& loop_rate)
{
	...
	robot.init_right_elbow();  //nikiforos 2025 se sxolio
	/*msg.data = std::string(CMD_START_RIGHT_ELBOW);
	ctl_pub.publish(msg);
