...
int readErr()
{
	FILE *f;
	int i;
	f = fopen("/home/mrrobot/nerr.txt","r");  //isos thelei allagh afto NIKHFOROS
	if (f == NULL) return 0;
...
int main(int argc, char** argv)
{
	...
	double rate;
	ros::param::param<double>("~loop_rate", rate, 200); //200 hz anti gia palio 500 (nikiforos 2025)
...
//	right_elbow_pub =  nh.advertise<std_msgs::Float64>("right_elbow_position_controller/command", 1000);
	...
	robot.setHomePos(7, r2_limit_pos); //se sxolio nikiforos 2025 gia na do arxikopoihsh robot
	...
	start_standard_controllers(nh, cm, loop_rate); //THA TO ANOIKSO
	...
	// ROS_WARN("controllers started and loaded, switching to effort..");
	// std_msgs::String msgforos;
	// msgforos.data = std::string(CMD_SWITCH_TO_EFFORT);
	// ctl_pub.publish(msgforos);
	

	sleep(1);
	// ROS_WARN("switched to effort");
	...
	//before spinning, set all commands to zero torque for arm (nikiforos 2025)
	//robot.setCmd(LEFT_SHOULDER, -0.0001);
	//robot.setCmd(LEFT_ELBOW, 0.0001);
	//robot.setCmd(RIGHT_ELBOW, -0.0001);

	//dokimi na kano kanoniko spinner, nikiforos 2025
	ROS_WARN("About to enter normal spinning...");
	ros::AsyncSpinner spinner(3);
	spinner.start(); //nikiforos to kleino gia ligo
	...
	while(!g_request_shutdown) {
		...
		//to anoigo, nikiforos 2025
		// robot.setCmd(LEFT_SHOULDER, 0.00000001);
		// robot.setCmd(LEFT_ELBOW, 0.00000001);
		// robot.setCmd(RIGHT_ELBOW, 0.000015);
		// ros::spinOnce();
