...
Eigen::Vector3d fext(0, 0,
                     0); // = Eigen::VectorXd::Zero(3); // << (0,0,0); //fx,fy,n
Eigen::Vector3d xee(0, 0, 0);    // = Eigen::VectorXd::Zero(3); //(0,0,0) //the
...
double xE_in, yE_in;
double q01 = 0; // to eftiaksa oste na einai mhden!  // -27.88931 * M_PI/180;
                // //gonia tou shoulder joint se sxesh me base
...
double offsetq2 = 0.0;
double offsetq3 = 0.0;
double q1known = 1.84447; // gonies analoga me tin gnosti diataksi, ego ekana
                          // omos thetiko akro, agonas karpos arnitika akra
double q2known = -0.9542;
double q3known = -0.8393; // palia itan -0.2856 omos allakse otan lysame tin
                          // triti arthrosi kai tin ksanadesame;
...
std::deque<double> q3dot_window; // Stores tons.h:304:23: error: ‘msg’ was not declaredhe last N values
