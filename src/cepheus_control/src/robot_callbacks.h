...
void ee_posCallback(const geometry_msgs::TransformStamped::ConstPtr &msg) {
    ...
    if (firstTime) {
        thetach = yawee;
    } else if (abs(yawee - thetach) < 8 * M_PI / 180) {
...
void target_posCallback(const geometry_msgs::TransformStamped::ConstPtr &msg) {
    ...
    if (firstTime) {
        thetat = yawt;
    } else if (abs(yawt - thetat) < 8 * M_PI / 180) {
...
void base_posCallback(const geometry_msgs::TransformStamped::ConstPtr &msg) {
    ...
    tf::Quaternion qc0(msg->transform.rotation.x, msg->transform.rotation.y,
                       msg->transform.rotation.z, msg->transform.rotation.w);
    ...
    if (firstTime) {
        theta0 = yawc0;
    } else if (abs(yawc0 - theta0) < 8 * M_PI / 180) {
        theta0 = yawc0; // angle of base
    }
}
